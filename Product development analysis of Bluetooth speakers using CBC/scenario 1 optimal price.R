...
# We can start with some initial price for one firm
# Let's assume firm 1 sets the price first, e.g., at 90EUR
# Firm 2 will respondent by setting a price which optimizes its profits 
...
# set initial prices
# 90 for firm 1, NA for the focal firm (we need to find the optimal 
# price for this firm), and 0 for alt 3, i.e., none
# If you have more options you can extend this vector
init_price = c(90, NA, 0) 
...
price_vec = c(90, NA, 0) # initial prices
...
# iteration 2 ---
# Firm 1 reacts to the price 85 of firm 2
# update the price_vec, focal firm, and the price seq
...
N = 400                  # define market size
mc = c(75, 70, 0)        # define the marginal costs
price_vec = c(40, NA, 0) # initial prices
...
#taking the initial value of price for firm 1 equal to the minimum cost of firm 1
# the loop will stop when max_iter is reached or earlier 
# when the difference between prices in r and r-1 is 0
for(r in 1:max_iter){
  if(r == 1){
...
# Doesn't seem to converge. we see that the minimum difference between prices we reach is 2
# In this case, the min_diff = 0 might be too low. We can set it to 2 for reaching convergence
# of course, you may be more detailed and generate prices with decimal points.
